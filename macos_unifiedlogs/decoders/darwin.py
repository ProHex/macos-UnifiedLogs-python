...
"""Darwin-specific decoders for errno codes and permissions."""

import logging

logger = logging.getLogger(__name__)

# Darwin errno codes from https://github.com/apple/darwin-xnu/blob/main/bsd/sys/errno.h
ERRNO_CODES = {
    "0": "Success",
...
def errno_codes(errno: str) -> str:
    """Convert Darwin errno codes to message.

    Args:
        errno: Error number as string

    Returns:
        Human-readable error message
    """
    if errno in ERRNO_CODES:
        return ERRNO_CODES[errno]

    logger.warning(f"[macos-unifiedlogs] Unknown darwin errno code: {errno}")
    return f"Unknown errno: {errno}"


def permission(permissions: str) -> str:
    ...
    for bit in permissions:
        message += permission_map.get(bit, "---")
